...
bool buscar_patente(hash_t* hash, const char* clave, void* aux){
  if(!clave)
    return true;
  ...
  bool recorrer = false;
  //printf("La clave a comparar es %p\n",extra);
  if(strcmp(aux,clave) == 0){
    printf("Son iguales\n");
    recorrer = true;
  }
...
int main(){
  ...
  size_t impresas_2 = hash_con_cada_clave(garage, buscar_patente,"AC152AD");
  printf("Se itera%i\n",impresas_2);
