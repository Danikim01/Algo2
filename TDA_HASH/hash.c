...
hash_t* hash_crear(hash_destruir_dato_t destruir_elemento, size_t capacidad_inicial){
	...
	if(capacidad_inicial >= MINIMA_CAPACIDAD_HASH){
		...
		hash->capacidad = MINIMA_CAPACIDAD_HASH;
	}
...
size_t funcion_hash(const char* clave,size_t capacidad){
	size_t hash = 0, pivot1 = 378551, pivot2 = 63689;
...
bool misma_clave(hash_t* hash, const char* clave, size_t posicion){
...
size_t obtener_posicion_valida(hash_t* hash, const char *clave){
